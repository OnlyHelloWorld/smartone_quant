...
from app.services.qmt_stock_weekly_service import sync_stock_weekly_klines_to_db
...
class TestQmtStockWeeklyService(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    @patch('app.services.qmt_stock_weekly_service.xtdata')
    def test_sync_stock_weekly_klines_to_db(self, mock_xtdata):
        """测试同步周K数据到数据库的服务"""
        # 模拟 xtdata.get_market_data 的返回值
        mock_data = {
            'time': [1622476800, 1623081600],  # 2021-06-01, 2021-06-08
            'open': [10.0, 10.5],
            'high': [11.0, 11.5],
            'low': [9.0, 9.5],
            'close': [10.5, 11.0],
