...
def sync_stock_daily_klines_to_db(
    ...
    try:
        ...
        if not daily_data:
            ...
            logger.info(f"获取到股票{stock_code}的月K数据，共{len(daily_data['time'])}条记录")
        ...
        try:
            ...
            logger.info(f"成功同步股票{stock_code}的月K数据，共尝试插入{affected_rows}条记录（唯一索引冲突时自动更新）")
        except Exception as e:
            logger.error(f"插入月K数据失败: {e}")
            raise
