...
from utils.db_utils import insert_on_duplicate_update_for_kline
...
def sync_stock_daily_klines_to_db(
    ...
    try:
        ...
        if not daily_data:
            ...
            logger.info(f"获取到股票{stock_code}的周K数据，共{len(daily_data['time'])}条记录")

        # 格式化数据
        stock_objs = parse_stock_data(daily_data, model_cls=QmtStockWeeklyOri)
        ...
        try:
            ...
            logger.info(f"成功同步股票{stock_code}的周K数据，共尝试插入{affected_rows}条记录（唯一索引冲突时自动更新）")
        except Exception as e:
            logger.error(f"插入周K数据失败: {e}")
            raise
...
def batch_download_all_stocks(stock_codes: List[str], start_time_str: str, end_time_str: str):
    ...
    try:
        # 打印函数入参
        logger.info(f'批量下载股票代码列表: {len(stock_codes)},开始时间: {start_time_str}, 结束时间: {end_time_str}')
